...
#     A_coordinate = (locationA.latitude, locationA.longitude)
#     B_coordinate = (locationB.latitude, locationB.longitude)
#     distance = geodesic(A_coordinate, B_coordinate).miles
#     return distance
...
def find_similarity(city_data, cityB, data_name):
    if cityB in CN_cities_list: 
        country_data = CN_data
    else:
...
app.layout = html.Div([
    # Title 
    html.H1(children = 'Cities with Similar Weather in China and the U.S.',
            style = {
                    'textAlign': 'center',
                    'color': rblue,
                    'fontsize': 20,
...
def update_output2(n_clicks, cityA_input):
    ...
    # Map 
#     if countryA == 'US':
#         coord_A = eval(US_coord[US_coord['city'].str.contains(cityA)].iloc[0,3])
#         coord_B = eval(CN_coord[CN_coord['asciiname'].str.contains(cityB_result)].iloc[0,3])
#     else:
#         coord_A = eval(CN_coord[CN_coord['asciiname'].str.contains(cityA)].iloc[0,3])
#         coord_B = eval(US_coord[US_coord['city'].str.contains(cityB_result)].iloc[0,3])
